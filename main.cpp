...
class BinSearchChecher {
   public:
    ...
    void LowerBoundCheck(const std::vector<T>& arr, const T& value) const {
        auto it = std::lower_bound(arr.begin(), arr.end(), value, comp_);
        out_ << "Lower bound for value " << value << " is " << *it << '\n';
    }
    void UpperBoundCheck(const std::vector<T>& arr, const T& value) const {
...
int main(int, char**) {
    ...
    BinSearchChecher<int> checker(std::cout);
